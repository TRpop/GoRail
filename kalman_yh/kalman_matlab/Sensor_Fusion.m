...
save_st = zeros(step,3);
save_sf = zeros(step,3);
...
for i=1:step
   ...
   %% gps
    ...
    gps_y = real_y + rand_gps_y/3*randn();
    gps_z = real_z + rand_gps_z/3*randn();
   ...
   %% kalman filter
    ...
    z = [sf_xyz(1); sf_xyz(2)]; % sensor fusion data
...
error_gps = mean(abs(save_real-save_gps))
error_st = mean(abs(save_real-save_st))
error_sf = mean(abs(save_real-save_sf))
error_kal = mean(abs(save_real-save_kal))
